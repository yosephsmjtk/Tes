...
function separateFruitsByType() {
    var _a;
    const fruitMap = new Map();
    for (const fruit of fruits) {
        const fruitType = fruit.fruitType;
        if (fruitMap.has(fruitType)) {
            (_a = fruitMap.get(fruitType)) === null || _a === void 0 ? void 0 : _a.push(fruit.fruitName);
        }
        else {
...
fruitTypesMap.forEach((fruits, fruitType) => {
    ...
    console.log(`Soal Nomor 2b: Wadah dengan tipe buah ${fruitType}: ${fruitList}`);
});
// Code Case 1 Soal No 3
function calculateTotalStockByType() {
...
totalStockByType.forEach((totalStock, fruitType) => {
    console.log(`Soal Nomor 3: Total stok buah pada wadah dengan tipe ${fruitType}: ${totalStock}`);
});
// Jawaban Case 1 Soal No 4
console.log("Soal nomor 4: Untuk saat ini saya belum memiliki komentar terhadap kasus ini, Terimakasih 😊 ");
